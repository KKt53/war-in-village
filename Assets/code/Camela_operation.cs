...
public class Camela_operation : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKey(KeyCode.I)) //Iキーが押されていれば
        {
            if (tf.position.x >= -5)
            {
                tf.position = tf.position + new Vector3(-0.01f, 0.0f, 0.0f); //カメラを左へ移動。
            }
        }
        else if (Input.GetKey(KeyCode.O)) //Oキーが押されていれば
        {
            if (tf.position.x <= 5)
            {
                tf.position = tf.position + new Vector3(0.01f, 0.0f, 0.0f); //カメラを右へ移動。
            }
        }
        ...
        else if (Input.GetKey(KeyCode.L)) //下キーが押されていれば
        {
            ...
            {
                cam.orthographicSize = cam.orthographicSize - 0.001f; //ズームイン。
            }
        }
    }
}
